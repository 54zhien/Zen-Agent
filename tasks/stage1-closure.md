...
This file records what Stage 1 delivered and the evidence behind each claim.
The executable gate evidence is `Tests/ZenAgentTests/Stage1GateTests.swift`;
everything below is the index that points at it and at the per-item CI runs.
...
The first Gate implementation run, `35589060462`, failed because its test fixture
encoded SSE frames without the blank-line event boundary. This was a
test-fixture / specification defect, not production RED evidence. Production SSE
truncation semantics were left unchanged. The fixture was corrected without
...
- test-run starts: 1
- test-host restarts: 0
- result: 269 tests / 32 suites passed
...
The authoritative Blueprint decision is `ac765ab57554e16ae2abbe017ca80c21ef8bedfe`.
...
Stage 3 must re-read this policy before implementing physical attachment storage.
A new product decision is required only if a later implementation intends to
change this policy.
...
Do not begin Stage 2 from this closure change. Stage 2 starts only after a new
explicit planning / review round.
